...
/* 找出红黑树中最左边的节点,如果它的超时时间大于当前时间,也就表明目前的定时器中没有一个
 * 事件满足触发条件,这是返回这个超时时间与当前时间的差值,也就是需要经过多少毫秒会有事件超时触发
 * 如果它的超时时间小于或者等于当前时间,则返回0, 表示当时器汇总已经存在了超时事件
 */
ngx_msec_t
...
/* 检查定时器中的所有事件,按照红黑树关键字从小到大的顺序依次调用已经满足超时
 * 条件需要被触发的handler回调方法
 */
void
