...
ngx_master_process_cycle(ngx_cycle_t *cycle)
{
    ...
    for ( ;; ) {
        ...
        if (ngx_terminate) { /* 接收到sig term消息,需要关闭所有的进程 */
            if (delay == 0) {
                delay = 50;
            }
...
/*
 * 子进程管理,ngx_reap_child方法会遍历ngx_processes数组,检查每一个子进程的状态
 * 对于非正常退出的子进程会重新拉起
 */
static ngx_uint_t
