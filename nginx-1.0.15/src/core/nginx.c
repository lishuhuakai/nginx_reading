...
/* 在执行不重启服务器升级Nginx的操作时,老的Nginx进程会通过
 * 环境变量NGINX来传递需要打开的监听端口,新的Nginx进程会通过
 * ngx_add_inherited_sockets方法来使用已经打开的TCP监听端口
 */
static ngx_int_t
...
ngx_get_options(int argc, char *const *argv)
{
    ...
    for (i = 1; i < argc; i++) {
        ...
        while (*p) {
            ...
            case 'c':
                if (*p) {
                    ngx_conf_file = p; /* nginx配置文件所在的路径 */
                    goto next;
                }
...
ngx_process_options(ngx_cycle_t *cycle)
{
    ...
    /* 配置的路径一般为$prfix/conf/nginx.conf */
